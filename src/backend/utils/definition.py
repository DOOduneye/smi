...
class Definition():
    ...
    def __init__(self):
        self.url = "https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles="
    ...
    def get_definition(self, genre: str, ending: str='music') -> str:
        ...
        response = req.get(self.url + genre + ' ' + ending)
        data = response.json()
